...
export default function TicketDetailModal({
  ...
  const [approvingTickets, setApprovingTickets] = useState<Set<number>>(
    new Set(),
  );
            ...
            <>
              <div className='ticket-summary'>
                ...
                <div className='summary-items'>
                  {sortedStatusGroups.map(
                    ([statusName, { tickets: statusTickets, statusId }]) => (
                      <div
                        key={statusName}
                        className={`summary-item ${
                          statusId === 4 ? "summary-pending" : ""
                        }`}
                      >
                        <span className='summary-status'>{statusName}:</span>
                        <span className='summary-count'>
                          {statusTickets.length}件
                        </span>
                      </div>
                    ),
                  )}
                    ...
                    <div key={statusName} className='status-group'>
                      <h3
                        className={`status-group-header ${
                          statusId === 4 ? "status-pending-review" : ""
                        }`}
                      >
                        ...
                        <div key={ticket.id} className='ticket-card'>
                          ...
                          {ticket.status.id === 4 && (
                            <div className='ticket-actions'>
                              ...
                              >
                                {approvingTickets.has(ticket.id)
                                  ? "処理中..."
                                  : "審査通過"}
                              </button>
                            </div>
